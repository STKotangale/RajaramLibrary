...
public class GeneralMemberController {
    ...
    public ResponseEntity<ApiResponseDTO<GeneralMemberResponseDTO>> getGeneralMemberById(@PathVariable int id) {
        GeneralMemberResponseDTO member = generalMemberService.getGeneralMemberById(id);
        ApiResponseDTO<GeneralMemberResponseDTO> responseDTO = new ApiResponseDTO<>(true, "General member found", member, HttpStatus.OK.value());
        return ResponseEntity.status(HttpStatus.OK).body(responseDTO);
    }

    @GetMapping
    public ResponseEntity<ApiResponseDTO<List<GeneralMemberResponseDTO>>> getAllGeneralMembers() {
        List<GeneralMemberResponseDTO> members = generalMemberService.getAllGeneralMembers();
        ApiResponseDTO<List<GeneralMemberResponseDTO>> responseDTO = new ApiResponseDTO<>(true, "All general members retrieved", members, HttpStatus.OK.value());
        return ResponseEntity.status(HttpStatus.OK).body(responseDTO);
    }
