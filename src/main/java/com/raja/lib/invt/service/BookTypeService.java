...
public class BookTypeService {
	...
	public ApiResponseDTO<BookType> createBookType(BookTypeRequest request) {
		try {
			...
			return new ApiResponseDTO<>(true, "Book type created successfully", savedBookType,
					HttpStatus.CREATED.value());
		} catch (DataIntegrityViolationException e) {
			return new ApiResponseDTO<>(false, "Failed to create book type. Type name already exists.", null,
		...
		if (optionalBookType.isPresent()) {
			try {
				...
				return new ApiResponseDTO<>(false,
						"Cannot delete the book type because it is referenced by other records", null,
