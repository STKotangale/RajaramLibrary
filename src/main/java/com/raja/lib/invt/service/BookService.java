...
public class BookService {
	...
	public ApiResponseDTO<Book> createBook(BookRequest request) {
		try {
			...
			if (optionalAuthor.isPresent() && optionalPublication.isPresent() && optionalBookType.isPresent()
					&& optionalBookLanguage.isPresent()) {
				...
				return new ApiResponseDTO<>(false, "Author, Publication, Book Type, or Book Language not found", null,
						HttpStatus.NOT_FOUND.value());
			}
		} catch (DataIntegrityViolationException e) {
			return new ApiResponseDTO<>(false, "Book name already exists", null, HttpStatus.CONFLICT.value());
		} catch (Exception e) {
			return new ApiResponseDTO<>(false, "An error occurred while creating the book", null,
	...
	public ApiResponseDTO<Void> deleteBook(int id) {
	    ...
	    if (optionalBook.isPresent()) {
	        try {
	            ...
	            return new ApiResponseDTO<>(false, "Cannot delete the book because it is referenced by other records", null, HttpStatus.CONFLICT.value());
	        }
	    } else {
	        return new ApiResponseDTO<>(false, "Book not found", null, HttpStatus.NOT_FOUND.value());
	    }
