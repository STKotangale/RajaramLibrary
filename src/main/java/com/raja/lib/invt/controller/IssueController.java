...
public class IssueController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<ApiResponseDTO<Stock>> getIssue(@PathVariable int id) {
        try {
            Stock stock = stockService.getissueById(id);
            return ResponseEntity.ok(new ApiResponseDTO<>(true, "Stock retrieved successfully", stock, HttpStatus.OK.value()));
        } catch (NotFoundException e) {
    ...
    @GetMapping("")
    public ResponseEntity<ApiResponseDTO<List<Stock>>> getIssueAll() {
        List<Stock> stocks = stockService.getAllIsuee();
        return ResponseEntity.ok(new ApiResponseDTO<>(true, "Stocks retrieved successfully", stocks, HttpStatus.OK.value()));
    }
